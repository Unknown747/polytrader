...
function handleExport() {
  const url = `${import.meta.env.BASE_URL}api/portfolio/export?type=orders`;
...
export default function Orders() {
  const { data: orders, isLoading } = useListOrders({ query: { refetchInterval: 30000 } });
  ...
  const openCount = orders?.filter((o) => o.status === "open").length ?? 0;
  const filledCount = orders?.filter((o) => o.status === "filled").length ?? 0;
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Trade history and open orders · Auto-refreshes every 30s</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      {orders && orders.length > 0 && (
        <div className="flex gap-3">
          <div className="rounded-lg border border-border bg-card px-3 py-2 text-center">
            ...
            <div className="text-lg font-bold text-primary font-mono">{openCount}</div>
          </div>
          <div className="rounded-lg border border-border bg-card px-3 py-2 text-center">
            <div className="text-xs text-muted-foreground">Filled</div>
            <div className="text-lg font-bold text-yes font-mono">{filledCount}</div>
          </div>
          <div className="rounded-lg border border-border bg-card px-3 py-2 text-center">
            <div className="text-xs text-muted-foreground">Total</div>
            <div className="text-lg font-bold text-foreground font-mono">{orders.length}</div>
          </div>
        </div>
      )}
